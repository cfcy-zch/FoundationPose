...
class TinyFrame:
    ...
    async def quary_async(self, type:int, listener, pld=None, id:int=None):
        """ Send a query """
        (id, buf) = self._compose(type=type, pld=pld, id=id)
    ...
    async def send_async(self, type:int, pld=None, id:int=None):
        """ Like query, but with no listener """
        await self.quary_async(type=type, pld=pld, id=id, listener=None)
