        ...
        for q in qs:
            ...
            if new_msg is not None:
                ...
                if msgs:
                    ...
                    cv2.imshow("depth", resize_image(depth_vis,100))
                    cv2.imshow("color", resize_image(color,100))
                    cTime = time.time()
                    fps = 1 / (cTime - pTime)       #计算处理频率
                    pTime = cTime
                    #rate.sleep()
                    cv2.putText(color, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3,(255, 255, 255), 2) 
                    cv2.imshow("color_fps", color)
